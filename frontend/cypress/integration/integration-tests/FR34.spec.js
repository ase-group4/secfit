...
describe("Integration tests", () => {
    ...
    describe("FR34.2: The athlete should be able to create their own ingredients, with nutritional value specified with the amount of protein, fat and carbohydrates per 100 grams", () => {
        ...
        it("ingredients on page with correct values", () => {
            ...
            cy.get("#div-content").contains(ingredient1["name"]).parent().within(()=>{
                cy.contains("Calories:").parent().within(()=>{
                    cy.contains((ingredient1.carbohydrates*4 + ingredient1.fat*9 + ingredient1.protein*4).toString() + " kcal")
                })
                cy.contains("Protein:").parent().within(()=>{
                    cy.contains((ingredient1.protein).toString() + " g")
                })
                cy.contains("Fat:").parent().within(()=>{
                    cy.contains((ingredient1.fat).toString() + " g")
                })
                cy.contains("Carbohydrates:").parent().within(()=>{
                    cy.contains((ingredient1.carbohydrates).toString() + " g")
                })
            });
            cy.get("#div-content").contains(ingredient2["name"]).should("be.visible");
            cy.get("#div-content").contains(ingredient2["name"]).parent().within(()=>{
                cy.contains("Calories:").parent().within(()=>{
                    cy.contains((ingredient2.carbohydrates*4 + ingredient2.fat*9 + ingredient2.protein*4).toString() + " kcal")
                })
                cy.contains("Protein:").parent().within(()=>{
    ...
    describe("FR34.2: The athlete should be able to create their own ingredients, with nutritional value specified with the amount of protein, fat and carbohydrates per 100 grams", () => {
        ...
        it("view meal with ingredient", () => {
            ...
            cy.get("#div-content").contains(meal1.name).parent().parent().within(()=>{
                cy.contains("Calories: " + (ingredient1.carbohydrates*2*4 + ingredient1.fat*2*9 + ingredient1.protein*2*4).toString() + " kcal")
    ...
    describe("FR34.3: Remove ingredient from meal", () => {
        /*
        In agreement with Haakon Gunleiksrud per email we did not implement (and therefor do not test) the part of the FR concerning 
        removing an ingredient after meal logging.
        ...
        */
        ...
        it("view meal with ingredient", () => {
            ...
            cy.get("#div-content").contains(meal2.name).parent().parent().within(()=>{
                cy.contains("Calories: " + (ingredient2.carbohydrates*2*4 + ingredient2.fat*2*9 + ingredient2.protein*2*4).toString() + " kcal")
